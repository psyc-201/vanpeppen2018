...
correct_matrix_pre <- as.data.frame(
  lapply(pretest_accuracy_questions, function(q) {
    pretest_answers[[q]] == pretest_accuracy_key[[q]]
  })
)
names(correct_matrix_pre) <- pretest_accuracy_questions
# Convert the true/false into binary numbers (1 and 0)
correct_matrix <- correct_matrix_pre * 1L
names(correct_matrix) <- paste0(names(correct_matrix), "_score")
...
pretest_scored <- cbind(
  pretest_all_data,                 # original info + raw answers
  correct_matrix,                   # 0/1 item scores
  pretest_total_accuracy = pretest_total_accuracy,
  pretest_total_prop     = pretest_total_prop
)
# 
head(pretest_scored[
  , c("pretest_total_accuracy", "pretest_total_prop", "Q0_score", "Q2_score", "Q4_score")
])
```
...
# Convert the true/false into binary numbers (1 and 0)
correct_matrix_post <- correct_matrix_post * 1
...
Before running the descriptive statistics, confirmatory, and exploratory analyses, we will first load in the scored pretest and posttest data.

```{r}
# Combine the pretest and posttest scorred data into one data set
analysis_df <- pretest_scored %>%
  select(participant_id, condition, pretest_total_accuracy, pretest_total_prop) %>%
  left_join(
    posttest_scored %>%
      select(participant_id, posttest_total_accuracy, posttest_total_prop),
    by = "participant_id"
  )
