...
```{r}
## 1. Start from ALL sampled rows, only filter by the pattern in `response`
posttest_raw <- data_sampled %>%
  filter(
    !is.na(response),
    grepl('"q[0-9]+_(mc|effort)"', response)   # look inside the JSON string
  ) %>%
  select(participant_id, source_file, condition, response)

nrow(posttest_raw)
head(posttest_raw$response)
## 2. Parse JSON, extract key (q5_mc, q5_effort, ...) and value
...
# sanity check: see what keys we actually have
unique(posttest_keys$key)[1:20]
## 3. Keep only q1–q8 mc/effort keys (ignore q*_text, etc.) and pivot wide
...
posttest_all_data <- posttest_mc_effort %>%
  distinct() %>%
  pivot_wider(
    names_from  = key,    # q1_mc, q1_effort, ...
    values_from = value
  )
