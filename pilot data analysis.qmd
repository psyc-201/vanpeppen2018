...
```{r}
# Keep BOTH mc and mc_effort rows for the 80 sampled participants
posttest_mc_and_effort <- data_sampled %>%
  filter(
    task == "posttest",                                      
    grepl("^q[1-8]_(mc|effort)$", item)                      
  ) %>%
  select(participant_id, source_file, condition, item, response)
# Pivot to wide format
posttest_all_data <- posttest_mc_and_effort %>%
  pivot_wider(
    names_from  = item,
...
```{r}
# Posttest effort questions (the effort ratings)
effort_questions_post <- c("effort_q1", "effort_q2", "effort_q3", "effort_q4",
                           "effort_q5", "effort_q6", "effort_q7", "effort_q8")
# Take only those columns from posttest_answers
posttest_effort_data <- posttest_answers[effort_questions_post]
...
# Convert effort text → numeric 1–5 safely
effort_num_post <- as.data.frame(
  lapply(posttest_effort_data, function(x)
    as.numeric(factor(x, levels = effort_key, labels = 1:5)))
)
# Name them something like q1_mc_effort, q2_mc_effort, ...
names(effort_num_post) <- paste0(names(effort_num_post), "_effort")
...
ggplot(pretest_scored_effort, aes(x = pretest_effort_mean,
  ...
  labs(
    ...
    x = "Mean Pretest Mental Effort",
    y = "Pretest Accuracy (Proportion Correct)"
  ) +
  theme_minimal(base_size = 14)
```
...
```{r}
# Find all posttest effort columns
effort_cols_post <- grep("_effort$", names(posttest_scored_effort), value = TRUE)
# Compute mean posttest effort across all 8 items
posttest_scored_effort <- posttest_scored_effort %>%
  mutate(
    posttest_effort_mean = rowMeans(across(all_of(effort_cols_post)), na.rm = TRUE)
  )
summary(posttest_scored_effort$posttest_effort_mean)
# Correlation
cor.test(
  posttest_scored_effort$posttest_total_prop,
  posttest_scored_effort$posttest_effort_mean
)
```
